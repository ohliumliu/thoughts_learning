...
In general, we would like to have $\vec u^{(i)} = \{\sqrt\lambda\delta_{ij}\}_j$ and $v^{(i)} = \theta_0$\footnotemark\footnotetext{You must have realized that the choice of $v^{(i)} = \theta_0$ violates our original requirement that it should be independt of $\theta_i$. To simplify the discussion, let's assume $\theta_0$ is not to be optimized. Another way to put it is to shift $y$ by $\langle y\rangle$ before regression, then the model shouldn't involve the constant term.}.  
